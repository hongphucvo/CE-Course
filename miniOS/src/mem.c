...
addr_t alloc_mem(uint32_t size, struct pcb_t * proc) {
	...
	for(int i = 0 ; i < NUM_PAGES ; i++) {
		if(_mem_stat[i].proc == 0) count_pages++;
	}
	//counting number of available page
	if(count_pages >= num_pages && proc->bp + num_pages * PAGE_SIZE < RAM_SIZE) {
		mem_avail = 1;
	}
	...
	if (mem_avail) {
		...
		int takenPages = 0;
		for(int current_index = 0 ; current_index < NUM_PAGES ; current_index++) {
			if(_mem_stat[current_index].proc == 0) {
				_mem_stat[current_index].proc = proc->pid;
				_mem_stat[current_index].index = takenPages;
				
				
				addr_t frame_index = current_index << OFFSET_LEN;
				addr_t first_lv = get_first_lv(ret_mem + takenPages * PAGE_SIZE);
				addr_t second_lv = get_second_lv(ret_mem + takenPages * PAGE_SIZE);

				struct page_table_t * page_table = get_page_table(first_lv , proc->seg_table);
				if(page_table != NULL) {
					page_table->table[page_table->size].v_index = second_lv;
					page_table->table[page_table->size].p_index = frame_index >> OFFSET_LEN;
					page_table->size++;
				} else {
            	    ...
            	    pages->table[0].v_index = second_lv;
            	    pages->table[0].p_index = frame_index >> OFFSET_LEN;
				}



				takenPages++;
				if(takenPages == num_pages) {
					_mem_stat[current_index].next = -1;
					break;
				}
				else
				{
					...
					for(j=current_index+1; j<NUM_PAGES; j++){
						if(_mem_stat[j].proc==0){
							_mem_stat[current_index].next=j;
							break;
						}
					}
				}
